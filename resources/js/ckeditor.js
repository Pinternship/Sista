...
$('.editor').each(function() {
  ...
  if ($(el).data('simple-toolbar')) {
    options = simpleEditorConfig
  }

  if ($(el).data('editor') == 'inline') {
    editor = InlineEditor
  } else if ($(el).data('editor') == 'balloon') {
    editor = BalloonEditor
  } else if ($(el).data('editor') == 'document') {
    editor = DocumentEditor
    el = $(el).find('.document-editor__editable')[0]
  }

  editor
    .create(el, options)
    .then(editor => {
      if ($(el).closest('.editor').data('editor') == 'document') {
