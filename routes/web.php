...
// Route::get('/acceptemail', function() {
...
//     {
//         $message
//             ->from('vanika@unika.ac.id')
//             ->to('ilhamrisky21@gmail.com', 'John Smith')
//             ->subject('Welcome!');
//     });
...
Route::group(['middleware'=>'only_admin_access'], function(){
    ...
    Route::get('faculty-register', 'UserController@registerFaculty')->name('register_faculty');
    Route::post('faculty-register', 'UserController@registerFacultyPost');
});

Route::group(['middleware'=>'only_faculty_access'], function(){
    ...
    //Report
        Route::group(['prefix'=>'report'], function(){
            ...
            // Monthly report
            // View
            Route::get('monthly_faculty', 'ReportController@MonthlyReport')->name('report_monthly_faculty');
            Route::get('monthly_faculty/{application_id}/view', 'ReportController@viewMonthlyReport')->name('report_monthly_view_faculty');
            // Final report
            // View
            Route::get('final_faculty', 'ReportController@FinalReport')->name('report_final_faculty');
            Route::get('final_faculty/{application_id}/view', 'ReportController@viewFinalReport')->name('report_final_view_faculty');
            // PDF download
            Route::get('final_faculty/{application_id}/pdf', 'ReportController@pdfFinalReport')->name('report_final_pdf_faculty');
        });
});
...
Route::group(['prefix'=>'dashboard', 'middleware' => 'dashboard'], function(){
    ...
    Route::get('json_chart/{job_id}', 'DashboardController@json_chart')->name('dashboard_json_chart');

    Route::get('json_lulus/{faculty_id}', 'DashboardController@json_lulus')->name('dashboard_json_lulus');

    Route::get('json_top_student/{faculty_id}/{angkatan}', 'DashboardController@json_top_student')->name('dashboard_json_top_student');

    Route::get('json_certificate_progdi/{progdi_id}', 'DashboardController@json_certificate_progdi')->name('dashboard_json_certificate_progdi');

    Route::post('pdf/transkrip', 'DashboardController@transkripPdf')->name('transkrip_pdf');

    Route::post('pdf/skpi', 'DashboardController@skpiPdf')->name('transkrip_skpi');

    Route::get('premium-data', 'DashboardController@premium_data')->name('dashboard_premium_data');

    Route::get('applied-jobs', 'DashboardController@dashboard')->name('applied_jobs');

    Route::get('test/{faculty_id}', 'DashboardController@test')->name('test');
    ...
    Route::group(['middleware'=>'admin_agent_employer'], function(){
        ...
        Route::group(['prefix'=>'employer'], function(){
            ...
            Route::group(['prefix'=>'job'], function(){
                ...
                Route::post('edit/{job_id}', 'JobController@postupdatejob');
            ...
            Route::get('accept', 'UserController@acceptedApplicant')->name('accepted_applicant');

            // Route::get('accept/json', 'UserController@json')->name('accepted_applicant_json');

            Route::get('applicant/{application_id}/shortlist', 'UserController@makeShortList')->name('make_short_list');
            Route::get('applicant/{application_id}/removeshortlist', 'UserController@removeShortList')->name('remove_short_list');
            Route::get('applicant/{application_id}/acceptlist', 'UserController@acceptList')->name('make_accept_list');
            Route::get('applicant/{application_id}/removeacceptlist', 'UserController@removeAcceptList')->name('remove_accept_list');
        ...
        Route::group(['prefix'=>'cms'], function(){
            ...
            Route::get('post/{id}/{status}', 'PostController@statusChange')->name('post_delete');
            Route::get('/{id}/{status}', 'PostController@statusChange')->name('page_delete');
        });
    ...
    Route::group(['middleware'=>'admin_agent_employer'], function(){
        Route::group(['prefix'=>'report'], function(){
            ...
            // Report
            // Monthly report
            Route::get('monthly', 'ReportController@MonthlyReport')->name('report_monthly');
            Route::get('monthly_report_job', function (){
                return redirect(route('home'));
            });
            // Add
            Route::post('monthly_report_job', ['as' => 'monthly_report_job', 'uses'=>'ReportController@addMonthlyReport']);
            Route::get('monthly/{application_id}/add', 'ReportController@addMonthlyReport')->name('report_monthly_add');
            Route::post('monthly/{application_id}/add', 'ReportController@postMonthlyReport');
            // Edit
            Route::get('monthly/{application_id}/edit', 'ReportController@editMonthlyReport')->name('report_monthly_edit');
            Route::post('monthly/{application_id}/edit', 'ReportController@posteditMonthlyReport');
            // Delete
            Route::get('monthly/{application_id}/delete', 'ReportController@deleteMonthlyReport')->name('report_monthly_delete');
            // View
            Route::get('monthly/{application_id}/view', 'ReportController@viewMonthlyReport')->name('report_monthly_view');

            // Final report
            Route::get('final', 'ReportController@FinalReport')->name('report_final');
            Route::get('final_report_job', function (){
                return redirect(route('home'));
            });
            // Add
            Route::post('final_report_job', ['as' => 'final_report_job', 'uses'=>'ReportController@addFinalReport']);
            Route::get('final/{application_id}/add', 'ReportController@addFinalReport')->name('report_final_add');
            Route::post('final/{application_id}/add', 'ReportController@postFinalReport');
            // View
            Route::get('final/{application_id}/view', 'ReportController@viewFinalReport')->name('report_final_view');
            // Edit
            Route::get('final/{application_id}/edit', 'ReportController@editFinalReport')->name('report_final_edit');
            Route::post('final/{application_id}/edit', 'ReportController@posteditFinalReport');
            // Delete
            Route::get('final/{application_id}/delete', 'ReportController@deleteFinalReport')->name('report_final_delete');

            // PDF download
            Route::get('final/{application_id}/pdf', 'ReportController@pdfFinalReport')->name('report_final_pdf');
        });
    });
    ...
    Route::group(['middleware'=>'only_admin_access'], function(){
        Route::group(['prefix'=>'kerjasama'], function(){
            ...
            // list kerjasama
            // Kerjasama
            Route::get('listperusahaan', 'CooperationController@ListPerusahaan')->name('list_perusahaan');
            // Add
            Route::get('add', 'CooperationController@addListPerusahaan')->name('add_list_perusahaan');
            Route::post('add', 'CooperationController@postnewListPerusahaan');
            // Edit
            Route::get('{application_id}/edit', 'CooperationController@editListPerusahaan')->name('list_perusahaan_edit');
            Route::post('{application_id}/edit', 'CooperationController@posteditListPerusahaan');
            // Delete
            Route::get('{application_id}/delete', 'CooperationController@deleteListPerusahaan')->name('list_perusahaan_delete');
            // PDF download
            // Route::get('final/{application_id}/pdf', 'CooperationController@pdfFinalReport')->name('report_final_pdf');
        });
    });
    ...
    Route::group(['middleware'=>'only_admin_access'], function(){
        ...
        Route::group(['prefix'=>'categories'], function(){
            ...
            Route::get('/{id}/{status}', 'CategoriesController@statusChange')->name('category_delete');
        });
    ...
    Route::group(['prefix'=>'payments'], function() {
        ...
        Route::get('status_change/{id}/{status}', ['as'=>'status_change', 'uses' => 'PaymentController@markSuccess']);
    });

    //Costume SISTA
    Route::get('final/user', 'ReportController@userFinalReport')->name('report_final_user');
    Route::get('final/{application_id}/pdf', 'ReportController@pdfFinalReport')->name('report_final_pdf_user');
    ...
    Route::group(['prefix'=>'u'], function(){
        ...
        //Accept Job
        Route::get('applicant/{application_id}/acceptjob', 'UserController@acceptJob')->name('make_accept');
        Route::get('applicant/{application_id}/removeacceptjob', 'UserController@removeacceptJob')->name('remove_accept');
