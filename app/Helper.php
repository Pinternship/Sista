...
if (! function_exists('batas_judul')){
    function batas_judul($text = null, $limit = 5) {
        $text = strip_tags($text);
        if (str_word_count($text, 0) > $limit) {
            $words = str_word_count($text, 2);
            $pos = array_keys($words);
...
if (! function_exists('batas_isi')){
    function batas_isi($text = null, $limit = 10) {
        $text = strip_tags($text);
        if (str_word_count($text, 0) > $limit) {
            $words = str_word_count($text, 2);
            $pos = array_keys($words);
...
if (! function_exists('batas_job')){
    function batas_job($text = null, $limit = 1) {
        $text = strip_tags($text);
        if (str_word_count($text, 0) > $limit) {
            $words = str_word_count($text, 2);
            $pos = array_keys($words);
